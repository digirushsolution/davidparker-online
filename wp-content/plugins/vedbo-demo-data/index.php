<?php
/*
Plugin Name:    Vedbo Package Demo Data
...
class Vedbo_Data_Demo_Plugin_Class{
    ...
    public static $theme_name = 'vedbo';

    public static $demo_site = 'https://vedbo.co/';
    ...
    public static function isLocal(){
        ...
        if ( strpos($hostname, '.novaworks.net') !== false || strpos($hostname, 'vedbo.co') !== false ) {
            $is_local = true;
        }
        return $is_local;
    }
...
add_action('plugins_loaded', function(){
    ...
    if(strtolower($theme->get_template()) != 'vedbo'){

        add_action( 'admin_notices', function(){
            printf( __( '%1$s"Vedbo Package Demo Data" requires %3$s"vedbo"%4$s theme to be installed and activated. Please active %3$s"vedbo"%4$s to continue.%2$s', 'novaworks-demodata' ), '<div class="error"><p>', '</p></div>' ,'<strong>', '</strong>' );
        });
    ...
    Vedbo_Data_Demo_Plugin_Class::get_instance();
}, 999);
