    ...
    setTooltipPositionControl( isEditor ) {
        ...
        const setupFakePos = () => {
            this.elements.$tooltip.each(function (index, element){
                let $_elFake = jQuery('.elementor-element-fake.elementor-element-'+self.getID()+' .kitify-hotspot__tooltip[data-id='+element.getAttribute('data-id')+']');
                if(Kitify.isInViewport(element)){
