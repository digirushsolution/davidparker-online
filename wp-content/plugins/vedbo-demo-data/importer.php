...
//			}
    ...
    if( $this->setting_args['product'] && !empty( $demo_selected['product'] ) ) {
		...
		unset($this->running_import_elementor_conditions);
	...
	private function handling_importer_elementor_conditions( $conditions, $kit_settings ){
        ...
        $caches = get_option('kitify_theme_builder_conditions');
        if(empty($caches)){
            $caches = [];
        }
        ...
        foreach ($conditions as $type => $condition){
            ...
            if(!empty($value)){
                foreach ($value as $item => $roles){
                    $template_id = nova_import_get_post_id_by_slug( $item, 'elementor_library' );
                    $value_to_save = [];
                    if(!empty($template_id)){
                        if($type == 'single-page') {
                            foreach ($roles as $role) {
                                ...
                                $page_id = nova_import_get_post_id_by_slug($page_name, 'page');
                                if (!empty($page_id)) {
        ...
        update_option('kitify_theme_builder_conditions', $caches);
        ...
        $this->logger->info(
            __('Elementor Builder Location has been set', 'novaworks')
        );
