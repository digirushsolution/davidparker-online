...
class Kitify_Countdown_Timer extends Kitify_Base {
    ...
    protected function enqueue_addon_resources(){
        if(!kitify_settings()->is_combine_js_css()){
		    $this->add_script_depends( 'kitify-w__countdown-timer' );
		    if(!kitify()->is_optimized_css_mode()) {
			    wp_register_style( $this->get_name(), kitify()->plugin_url('assets/css/addons/countdown-timer.css'), ['kitify-base'], kitify()->get_version());
			    $this->add_style_depends( $this->get_name() );
		    }
	    }
    }
	public function get_widget_css_config($widget_name){
		$file_url = kitify()->plugin_url(  'assets/css/addons/countdown-timer.css' );
		$file_path = kitify()->plugin_path( 'assets/css/addons/countdown-timer.css' );
		return [
			'key' => $widget_name,
